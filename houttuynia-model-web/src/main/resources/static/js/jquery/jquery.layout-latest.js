...
(function (a) {
    ...
    a.ui.cookie = {
        acceptsCookies: !!navigator.cookieEnabled, read: function (b) {
            var h = document.cookie, e = h ? h.split(";") : [], g, f, d;
            for (d = 0; g = e[d]; d++) {
                f = a.trim(g).split("=");
                if (f[0] == b) {
    ...
    a.layout.state = {
        saveCookie: function (f, d, c) {
            var g = f.options, h = g.stateManagement, b = a.extend(true, {}, h.cookie, c || null),
                e = f.state.stateData = f.readState(d || h.stateKeys);
            ...
            var v = d.options.stateManagement, o = k.includeChildren, b = o !== undefined ? o : v.includeChildren,
                l = k.stateKeys || v.stateKeys, w = {isClosed: "initClosed", isHidden: "initHidden"}, e = d.state,
                j = a.layout.config.allPanes, x = {}, q, m, y, z, r, t, f, h, g, c;
            if (a.isArray(l)) {
                l = l.join(",")
            }
            ...
            var b = window.JSON || {};
            return (b.stringify || d)(c);

            function d(m) {
                var n = [], l = 0, g, f, j, e = a.isArray(m);
                for (g in m) {
                    f = m[g];
                    j = typeof f;
                    if (j == "string") {
...
(function (a) {
    ...
    a.layout.buttons = {
        config: {borderPanes: "north,south,west,east"}, init: function (d) {
            var e = "ui-layout-button-", c = d.options.name || "", b;
            a.each("toggle,open,close,pin,toggle-slide,open-slide".split(","), function (f, g) {
                a.each(a.layout.buttons.config.borderPanes.split(","), function (h, i) {
        ...
        }, get: function (f, b, h, e) {
            var c = a(b), g = f.options;
            if (c.length && a.layout.buttons.config.borderPanes.indexOf(h) >= 0) {
                var d = g[h].buttonClass + "-" + e;
                c.addClass(d + " " + d + "-" + h).data("layoutName", g.name)
            }
            return c
        }, bind: function (e, d, c, f) {
            ...
            switch (c.toLowerCase()) {
                ...
                case "pin":
                    ...
                    break;
                case "toggle-slide":
                    b.addToggle(e, d, f, true);
                    break;
                case "open-slide":
                    b.addOpen(e, d, f, true);
                    break
            }
            return e
            ...
            return c
        }, addPin: function (e, b, f) {
            var c = a.layout.buttons.get(e, b, f, "pin");
            if (c.length) {
                var d = e.state[f];
                c.click(function (g) {
                    a.layout.buttons.setPinState(e, a(this), f, (d.isSliding || d.isClosed));
                    if (d.isSliding || d.isClosed) {
                        e.open(f)
                    } else {
                        e.close(f)
                    }
                ...
                a.layout.buttons.setPinState(e, c, f, (!d.isClosed && !d.isSliding));
                d.pins.push(b)
            }
            return e
        }, setPinState: function (g, i, d, f) {
            var l = i.attr("pin");
            if (l && f === (l == "down")) {
                return
            }
            var e = g.options[d], c = e.tips, b = e.buttonClass + "-pin", h = b + "-" + d, k = b + "-up " + h + "-up",
                j = b + "-down " + h + "-down";
            i.attr("pin", f ? "down" : "up").attr("title", f ? c.Unpin : c.Pin).removeClass(f ? k : j).addClass(f ? j : k)
        }, syncPinBtns: function (c, d, b) {
            a.each(state[d].pins, function (f, e) {
                a.layout.buttons.setPinState(c, a(e), d, b)
            })
        }, _load: function (c) {
            a.extend(c, {
                bindButton: function (e, f, g) {
                    return a.layout.buttons.bind(c, e, f, g)
                }, addToggleBtn: function (f, g, e) {
                    return a.layout.buttons.addToggle(c, f, g, e)
                }, addOpenBtn: function (f, g, e) {
                    return a.layout.buttons.addOpen(c, f, g, e)
                }, addCloseBtn: function (e, f) {
                    return a.layout.buttons.addClose(c, e, f)
                }, addPinBtn: function (e, f) {
                    return a.layout.buttons.addPin(c, e, f)
                }
            });
            for (var b = 0; b < 4; b++) {
                var d = a.layout.buttons.config.borderPanes[b];
                c.state[d].pins = []
            }
            if (c.options.autoBindCustomButtons) {
                a.layout.buttons.init(c)
            }
        }, _unload: function (b) {
        }
    };
...
(function (a) {
    ...
    a.layout.browserZoom = {
        _init: function (b) {
            ...
            var k = window, m = screen, h = document, f = h.documentElement || h.body, i = a.layout.browser,
                l = i.version, c, e, j;
            if (!i.msie || l > 8) {
                return false
            }
...
(function (a) {
    if (a.effects) {
        a.layout.defaults.panes.useOffscreenClose = false;
        if (a.layout.plugins) {
            a.layout.plugins.effects.slideOffscreen = true
        }
        a.layout.effects.slideOffscreen = a.extend(true, {}, a.layout.effects.slide);
        a.effects.slideOffscreen = function (b) {
            return this.queue(function () {
                var h = a.effects, d = b.options, t = a(this), f = t.data("layoutEdge"),
                    c = t.data("parentLayout").state, m = c[f].size, r = this.style,
                    n = ["top", "bottom", "left", "right"], j = h.setMode(t, d.mode || "show"), q = (j == "show"),
                    g = d.direction || "left", e = (g == "up" || g == "down") ? "top" : "left",
                    o = (g == "up" || g == "left"), p = a.layout.config.offscreenCSS || {},
                    l = a.layout.config.offscreenReset, k = "offscreenResetTop", i = {};
                i[e] = (q ? (o ? "+=" : "-=") : (o ? "-=" : "+=")) + m;
                if (q) {
                    t.data(k, {top: r.top, bottom: r.bottom});
                    if (o) {
                        t.css(e, isNaN(m) ? "-" + m : -m)
                    } else {
                        if (g === "right") {
                            t.css({left: c.container.layoutWidth, right: "auto"})
                        } else {
                            t.css({top: c.container.layoutHeight, bottom: "auto"})
                        }
                    }
                    if (e === "top") {
                        t.css(t.data(l) || {})
                    ...
                    t.data(k, {top: r.top, bottom: r.bottom});
                    t.data(l, {left: r.left, right: r.right})
                }
                t.show().animate(i, {
                    queue: false, duration: b.duration, easing: d.easing, complete: function () {
                        if (t.data(k)) {
                            t.css(t.data(k)).removeData(k)
                        }
                        if (q) {
                            t.css(t.data(l) || {}).removeData(l)
                        } else {
                            t.css(p)
                        }
                        if (b.callback) {
