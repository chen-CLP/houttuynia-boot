(function (a) {
    ...
    a.extend(a.validator.defaults, {
        ...
        errorPlacement: function (b, c) {
            ...
            } else if (c.hasClass("bootstrap-select")) {
                c = c.prev();
                c.css("position", "relative")
            } else {
...
jQuery.validator.addMethod("phone",
    function (c, b) {
        var a = /(^0[1-9]{1}\d{8,10}$)|(^1[3,4,5,7,8]\d{9}$)/g;
        return this.optional(b) || (a.test(c))
    },
...
jQuery.validator.addMethod("idcard",
    function (b, a) {
        var c = function (g) {
            g = g.toString();
            var i = [true, false, false, false, false];
            var h = {
                11: "北京",
                12: "天津",
                13: "河北",
...
jQuery.validator.addMethod("checkLbCode",
    function (b, a) {
        return this.optional(a) || /^(([0-9]{3})|([0-9]{3})+(\.[0-9]{3})*)$/.test(b)
    },
    $.validator.messages.checkLbCode);
jQuery.validator.addMethod("checkJssorNum",
    function (b, a) {
        return this.optional(a) || /^([0-9]{3})$/.test(b)
    },
    $.validator.messages.checkJssorNum);
